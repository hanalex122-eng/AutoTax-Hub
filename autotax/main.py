...
async def upload_invoice(file: UploadFile = File(...), handwriting: bool = False, user: dict = Depends(get_current_user)):
    ...
    try:
        raw_text, qr_data = await asyncio.wait_for(extract_text_and_qr(file, handwriting=handwriting), timeout=18)
...
async def upload_batch(files: List[UploadFile] = File(...), user: dict = Depends(get_current_user)):
    ...
    for file in files:
        try:
            ...
            await file.seek(0)
            try:
                raw_text = await asyncio.wait_for(extract_text(file, handwriting=False), timeout=18)
            except Exception:
